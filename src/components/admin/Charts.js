...
function Charts () {
  ...
  const StatsUserWeek = async() => {
    let result = await axios.get(`http://localhost:3001/admin/stats/perweek?model=UserModel`, {
      headers: {
  ...
  const StatsShoppingWeek = async() => {
    let result = await axios.get(`http://localhost:3001/admin/stats/perweek?model=ShoppingOrder`, {
      headers: {
        Authorization: `Bearer ${JSON.parse(token)}`,
        "Content-Type": "application/json, charset=utf-8",
      },
  ...
  const ZonesPie = async () => {
    let result = await axios.get('http://localhost:3001/admin/stats/wines/fields?origin=1', {
      headers: {
        Authorization: `Bearer ${JSON.parse(token)}`,
        "Content-Type": "application/json, charset=utf-8",
      }})
  ...
  //   let result = await axios.get(`http://localhost:3001/admin/stats/permonth?model=ShoppingOrder`, {
  //     headers: {
  //       Authorization: `Bearer ${JSON.parse(token)}`,
  //       "Content-Type": "application/json, charset=utf-8",
  //     },
  ...
  if (userDataWeek === 'loading' || shoppingDataWeek === 'loading' || pieOrigin === 'loading') return <p>Loading...</p> 
  ...
  <Grid item lg={6} md={12} xs={11}>
    <Grid container>
      <Grid item lg={6} md={12} xs={12}>
        <CardUser title={"New Users this month"} amount={25} icon={<GroupAdd sx={{ fontSize: 40 }}/>}/>
